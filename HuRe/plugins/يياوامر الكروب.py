...
@l313l.on(events.NewMessage(pattern=r"^حماية التفليش$", chats=events.ChatAction))
async def toggle_listen(event):
    global group_id, listening_enabled

    if event.is_group and event.chat_id > 0:
        if group_id is None or not listening_enabled:
            group_id = event.chat_id
            listening_enabled = True
            await event.reply(f"Enabled group monitoring for group ID: {group_id}")
            l313l.add_event_handler(handle_event, events.NewMessage(chats=group_id))
        else:
            listening_enabled = False
...
async def handle_event(event):
    ...
    if event.message.from_id in await l313l.get_participants(group_id, filter=ChannelParticipantsAdmins):
        if "kicked" in event.raw_text:
            kicked_count += 1
            if kicked_count == 3:
                print("Detected 3 members being kicked within the time limit.")
                await l313l(EditAdminRequest(group_id, event.message.from_id, change_info=False,
